...
def normalized_columns_initializer(weights, device, std=1.0):
    out = torch.randn(weights.size()).to(device)
...
class ActorCritic(torch.nn.Module):
    ...
    def forward(self, inputs):
        state , (hx, cx) = inputs
        rgb = state[:,0:3,:,:]
        depth = torch.unsqueeze(state[:,3,:,:], dim = 1)
        ...
        #combined_output = torch.flatten(torch.cat((rgb_output,depth_output), dim = 1), start_dim = 1)
        rgb_output = torch.flatten(rgb_output, start_dim = 1)
        hx, cx = self.lstm(rgb_output, (hx, cx))
        #hx, cx = self.lstm(combined_output, (hx, cx))
        x = hx
